...
<div class="container">
 ...
 <h2 id="text_of_user" style="color:black;position: relative; top: 10px;"><b>{{strtoupper($user->firstname)}}</b><b>{{ ucwords($user->lastname )}}</b></h2>
   ...
   <div class="panel">
     <ul class="well">  
       <li><a href="#" class="fetch_all_photo" id="{{$user->id}}">Photo</a></li>  
      <li><a href="#" class="fetch_all_video" id="{{$user->id}}">Video</a></li>  
     ...
     <a href="/showAll_image"> <img src="/uploads/avatar/{{$user->avatar}}" style="width:50px; height:50px; float:left; border-radius:50%;  "></a>
          ...
          $tag_na = DB::table('users')->where('id',$tag[$i])->get();
              ...
              @if($post->tag_name != 0)
                  ...
                  <h2 id="text_of_user" style="color:black;position: relative; top: -10px;"><b> @if(strlen($user->firstname) <=12) {{ucwords($user->firstname )}} {{ucwords($user->lastname )}}@else {{substr(ucwords($user->firstname),0,12)}}..{{substr(ucwords($user->lastname),0,12)}}   @endif </b> <b style="color:blue"> is with  </b>     <?php    
              $tag = explode(",",$post->tag_name);
               $tag_count = count($tag);
                  if($tag_count > 3){ 
                      for ($i=0; $i <= 1; $i++) { 
   ...
   if($i == 0)
{
  ...
  ?> 
        ...
        <!-- <b style="font-size:14px;color:black;cursor: pointer;" class="edit_tag_show" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">{{$post->post_option}}</b>   -->
        <img src="upload/image/world_image.png" style="position: relative;top:5px;cursor: pointer;" width="20px" class="edit_tag_show" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">
        @endif
   @endif
       @if($post->tag_name != 0)
       @if($post->post_option == 'public')
         <!-- <b style="font-size:14px;color:gray;cursor: pointer;" class="edit_tag_show_friend" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">{{$post->post_option}}</b> -->
         <img src="upload/image/world_image.png" style="position: relative;top:5px;cursor: pointer;" width="20px" class="edit_tag_show_friend" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">
      @endif
       @endif
        @if($post->tag_name == 0)
          @if($post->post_option == 'private')
        <!-- <b style="font-size:14px;color:black;cursor: pointer;" class="edit_tag_show" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">{{$post->post_option}}</b>   -->
        <img src="upload/image/private_image.png" style="position: relative;top:5px;cursor: pointer;" width="20px" class="edit_tag_show" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">
        @endif
   @endif
       @if($post->tag_name != 0)
       @if($post->post_option == 'private')
         <!-- <b style="font-size:14px;color:gray;cursor: pointer;" class="edit_tag_show_friend" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">{{$post->post_option}}</b> -->
         <img src="upload/image/private_image.png" style="position: relative;top:5px;cursor: pointer;" width="20px" class="edit_tag_show_friend" edit_post="{{$post->id}}" onclick="document.getElementById('id01').style.display='block'">
      @endif
       @endif
          <!-- <span class="caret"  id="edit_tag_show" value="{{$post->id}}"></span>   -->
          
           <!-- <div id="dropdown_tag" style="display: none;">
             <a onclick="">Edit</a>
           </div> -->
                   <br>
<!-- <div class="cancel_edit_tag"> </div>-->
...
@if($post->post_option == 'public')
 ...
 <img src="/uploads/avatar/{{$tag_named->avatar}}" style="width:50px; height:50px; float:left; border-radius:50%; cursor: pointer;" class="friend_profile"value="{{$tag_named->id}}"> 
               
                  <h2 id="text_of_user" style="color:black;position: relative; top: -10px;"><b style="cursor: pointer;" class="friend_profile"value="{{$tag_named->id}}"> @if(strlen($tag_named->firstname) <=12) {{ucwords($tag_named->firstname )}}{{ucwords($tag_named->lastname )}}@else {{substr(ucwords($tag_named->firstname),0,12)}}..{{substr(ucwords($tag_named->lastname),0,12)}}   @endif </b> <b style="color:blue"> is with  </b>     <?php    
  ...
  foreach($tag_na as $name){
   ...
   ?>
    <a href="/showAll_image" style="text-decoration: none;">
   <b style="color:brown;font-size:20px;cursor: pointer;">
  
    @if(strlen($name->firstname) <=12) {{ucwords($name->firstname )}} {{ucwords($name->lastname )}}@else {{substr(ucwords($name->firstname),0,12)}}..{{substr(ucwords($name->lastname),0,12)}}   @endif </b></a> 
   

              <?php 
              if($tag_count > 1){
      echo "<b style='color:red;'>And +</b>";
       echo "<b style='color:green;cursor:pointer' class='tag_value' value=".$tag[$i].">".($tag_count-1)."</b>";
  }
  }
  ...
  ?> 
        ...
        @endforeach
                   ...
                   {{ $post->formattedCreatedDate() }} 
            <!--  <b style="font-size:14px;color:gray;cursor: pointer;"> {{$post->post_option}}</b> -->
...
<div class="panel">
  <div class="well">
    <h2 style="text-align: center;color:blue;">No Post</h2>
  </div>
</div>
@endif

<?php $che = DB::table('posts')->where('user_id',Auth::user()->id)->first();?>
@if($che == false)
<div class="panel">
  <div class="well">
    <h2 style="text-align: center;color:blue;">No Post</h2>
